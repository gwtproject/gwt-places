...
import net.ltgt.gwt.place.testplaces.Place5;
...
public class MostToLeastDerivedPlaceTypeComparatorTest {
  ...
  @Test public void testPlaceComparesGreaterThanAnyDerivedClass() {
    for (TypeElement p : new TypeElement[] { place1, place2, place3, place4, place5 }) {
      assertThat(comparator.compare(place, p)).named(String.format("compare(place, %s)", p)).isGreaterThan(0);
      assertThat(comparator.compare(p, place)).named(String.format("compare(%s, place)", p)).isLessThan(0);
    }
  }

  @Test public void testPlaceInheritanceOrder() {
  ...
  @Test public void testFallbackToClassName() {
    ...
    for (TypeElement[] pair : places) {
      assertThat(comparator.compare(pair[0], pair[1])).named(String.format("compare(%s, %s)", pair[0], pair[1])).isLessThan(
          0);
      assertThat(comparator.compare(pair[1], pair[0])).named(String.format("compare(%s, %s)", pair[1], pair[0])).isGreaterThan(
          0);
    }
  }
