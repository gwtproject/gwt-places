...
public class PlaceController {
  ...
  /**
   * Optional delegate in charge of Window-related events. Provides nice isolation for unit testing,
   * and allows customization of confirmation handling.
   */
  ...
  public PlaceController(EventBus eventBus, Delegate delegate) {
    ...
    delegate.addWindowClosingHandler(event -> event.setMessage(maybeGoTo(Place.NOWHERE)));
  }
  ...
  /**
   * Request a change to a new place. It is not a given that we'll actually get there. First a
   * {@link PlaceChangeRequestEvent} will be posted to the event bus. If any receivers post a
   * warning message to that event, it will be presented to the user via {@link
   * Delegate#confirm(String)} (which is typically a call to {@link Window#confirm(String)}). If she
   * cancels, the current location will not change. Otherwise, the location changes and a {@link
   * PlaceChangeEvent} is posted announcing the new place.
   *
