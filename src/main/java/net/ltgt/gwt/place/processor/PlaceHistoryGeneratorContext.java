...
class PlaceHistoryGeneratorContext {
  ...
  static PlaceHistoryGeneratorContext create(Messager messager, Types types, Elements elements, Element element) {
    ...
    TypeElement placeHistoryMapperWithFactoryType = requireType(elements, PlaceHistoryMapperWithFactory.class);

    if (element.getKind() != ElementKind.INTERFACE) {
      messager.printMessage(Diagnostic.Kind.WARNING, String.format("%s applied on a type that's not an interface; ignoring.",
          WithTokenizers.class.getCanonicalName()));
      return null;
    }
    if (!types.isSubtype(element.asType(), placeHistoryMapperType.asType())) {
      messager.printMessage(Diagnostic.Kind.WARNING, String.format("%s applied on a type that doesn't implement %s; ignoring.",
          WithTokenizers.class.getCanonicalName(), PlaceHistoryMapper.class.getCanonicalName()));
      return null;
    }

    TypeElement interfaceType = (TypeElement) element;
    DeclaredType factoryType = findFactoryType(types, placeHistoryMapperWithFactoryType, interfaceType);
    ClassName interfaceName = ClassName.get(interfaceType);
    String implName = ClassName.get(interfaceType).simpleNames().stream().collect(Collectors.joining("_")) + "Impl";
    return new PlaceHistoryGeneratorContext(messager, types, elements, interfaceType, factoryType, stringType, placeTokenizerType, interfaceName.packageName(), implName);
  }

  private static DeclaredType findFactoryType(Types types, TypeElement placeHistoryMapperWithFactoryType, TypeElement interfaceType) {
    if (!types.isSubtype(interfaceType.asType(), types.getDeclaredType(
        placeHistoryMapperWithFactoryType))) {
      return null;
    }
    return MoreTypes.asDeclared(types.asMemberOf((DeclaredType) interfaceType.asType(),
        placeHistoryMapperWithFactoryType.getTypeParameters().get(0)));
  }
  ...
  PlaceHistoryGeneratorContext(Messager messager, Types types, Elements elements, TypeElement interfaceType, DeclaredType factoryType, TypeElement stringType, TypeElement placeTokenizerType, String packageName, String implName) {
  ...
  public ExecutableElement getTokenizerGetter(String prefix) {
    ...
    return tokenizerGetter.getKind() == ElementKind.METHOD ? (ExecutableElement) tokenizerGetter : null;
  }
  ...
  private void addPlaceTokenizer(Element tokenizerClassOrGetter, String prefix, DeclaredType tokenizerType) {
    if (prefix.contains(":")) {
      this.messager.printMessage(Diagnostic.Kind.ERROR, String
          .format("Found place prefix \"%s\" containing separator char \":\", on %s",
              prefix, this.getLogMessage(tokenizerClassOrGetter)),
          tokenizerClassOrGetter);
    } else if (this.tokenizers.containsKey(prefix)) {
      this.messager.printMessage(Diagnostic.Kind.ERROR, String
          .format("Found duplicate place prefix \"%s\" on %s, already seen on %s",
              prefix, this.getLogMessage(tokenizerClassOrGetter),
      ...
      if (this.placeTypes.containsKey(placeType)) {
        this.messager.printMessage(Diagnostic.Kind.ERROR, String
            .format("Found duplicate tokenizer\'s place type \"%s\" on %s, already seen on %s",
                placeType.getQualifiedName(), this.getLogMessage(tokenizerClassOrGetter),
  ...
  private String getLogMessage(Element methodOrClass) {
    switch (methodOrClass.getKind()) {
      case METHOD:
        return ((TypeElement) methodOrClass.getEnclosingElement()).getQualifiedName() + "#" + methodOrClass.getSimpleName() + "()";
      case CLASS:
  ...
  private TypeElement getPlaceTypeForTokenizerType(DeclaredType tokenizerType) {
    TypeMirror placeType = types.asMemberOf(tokenizerType,
        this.placeTokenizerType.getTypeParameters().get(0));
    if (placeType.getKind() != TypeKind.DECLARED) {
      this.messager.printMessage(Diagnostic.Kind.ERROR,
          "Found no Place type for " + ((TypeElement) types.asElement(tokenizerType)).getQualifiedName());
      return null;
    } else {
  ...
  private Set<TypeElement> getWithTokenizerEntries() {
    Optional<AnnotationMirror> annotation = MoreElements
        .getAnnotationMirror(interfaceType, WithTokenizers.class);
    if (!annotation.isPresent()) {
      return Collections.emptySet();
    }
    @SuppressWarnings("unchecked") List<AnnotationValue> values =
        (List<AnnotationValue>) AnnotationMirrors.getAnnotationValue(annotation.get(), "value").getValue();
    LinkedHashSet<TypeElement> rtn = new LinkedHashSet<>();
    for (AnnotationValue value : values) {
      DeclaredType tokenizerType = (DeclaredType) value.getValue();
      rtn.add((TypeElement) tokenizerType.asElement());
    }
  ...
  private void initTokenizerGetters() {
    if (this.factoryType != null) {
      for (ExecutableElement method : ElementFilter.methodsIn(elements.getAllMembers((TypeElement) factoryType.asElement()))) {
        if (!method.getParameters().isEmpty()) {
          continue;
        }
        if (!method.getModifiers().contains(Modifier.PUBLIC)) {
          // TODO: support non-public but callable/visible methods (package-private or protected, when in same package)
          continue;
        }
        ...
        addPlaceTokenizer(method, getPrefixForTokenizerGetter(method, methodType), MoreTypes.asDeclared(returnType));
      }
    }
  }
  ...
  private void initTokenizersWithoutGetters() {
    for (TypeElement tokenizerType : this.getWithTokenizerEntries()) {
      ...
      this.addPlaceTokenizer(tokenizerType, this.getPrefixForTokenizerType(tokenizerDeclaredType), tokenizerDeclaredType);
    }
  }
}
