...
class MostToLeastDerivedPlaceTypeComparator implements Comparator<TypeElement> {
  private final Comparator<TypeElement> realComparator = Comparator
      .comparingInt(this::getHierarchyDepth).reversed()
  ...
  private int getHierarchyDepth(TypeElement typeElement) {
    ...
    if (depth == null) {
      if (typeElement.getSuperclass().getKind() == TypeKind.NONE) {
        ...
        depth = getHierarchyDepth((TypeElement) ((DeclaredType) typeElement.getSuperclass()).asElement()) + 1;
        hierarchyDepths.put(typeElement, depth);
      }
    }
    return depth;
